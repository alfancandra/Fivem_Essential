...
function onPlayerJoined(playerId)
	...
	if license then
		if ESX.GetPlayerFromIdentifier(license) then
			...
			MySQL.Async.fetchScalar('SELECT 1 FROM users WHERE license = @license', {
				...
				if result then
					...
					local inventory = {phone=1,bread=5,water=5}

					MySQL.Async.execute('INSERT INTO users (accounts, identifier,license,inventory) VALUES (@accounts, @identifier,@license,@inventory)', {
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo and itemCount > 0 then
					...
					TriggerEvent("esx:giveweaponalert",sourceXPlayer.name,targetXPlayer.name,weaponLabel,itemCount)
				else
					...
					TriggerEvent("esx:giveweaponalert",sourceXPlayer.name,targetXPlayer.name,weaponLabel,0)
				end
			else
